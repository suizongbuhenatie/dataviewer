...
class Header(Component):
    ...
    # 预计算样式映射
    _ALIGN_MAP = {"left": "text-left", "center": "text-center", "right": "text-right"}
    ...
    def __post_init__(self):
        ...
        if self.color not in self._COLOR_MAP:
            raise ValueError(
                f"不支持的颜色: {self.color}，支持的颜色有: {', '.join(self._COLOR_MAP.keys())}"
            )
...
class Tag(Component):
    ...
    _BASE_CLASSES = ["inline-flex", "items-center", "rounded-full", "font-medium"]

    def __init__(
        self, text: str, color: str = "blue", size: str = "md", id: Optional[str] = None
    ):
        self.text = text
        self.color = color
        self.size = size
    ...
    def to_html(self) -> str:
        ...
        class_str = " ".join(classes)
        return '<span id="%s" class="%s">%s</span>' % (self.id, class_str, self.text)
