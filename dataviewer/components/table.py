...
class Table(Component):
    ...
    def __init__(
        ...
        self.min_column_width = min_column_width if min_column_width is not None else 100
        self.max_column_width = max_column_width if max_column_width is not None else 300
    ...
    def _get_column_type(self, value: Any) -> Tuple[bool, bool]:
        ...
        is_image_array = isinstance(value, list) and any(
            ...
            CellRendererRegistry.render(item) and 
            "<img" in CellRendererRegistry.render(item) 
            for item in value
        )
    ...
    def _infer_columns(self) -> None:
        ...
        self.columns = [{"key": key, "title": key.replace('_', ' ').title()} for key in all_keys]
            ...
            for key in all_keys:
                if key not in self.column_widths:
                    ...
                    if not (is_image or is_image_array):
                        self.column_widths[key] = f"clamp({self.min_column_width}px, auto, {self.max_column_width}px)"
    ...
    def _get_display_data(self) -> List[Dict[str, Any]]:
        ...
        if self.sort_by:
            data = sorted(self.data, key=lambda x: x.get(self.sort_by, ""), reverse=self.sort_desc)
        else:
            data = self.data
    ...
    def _render_cell(self, value: Any, is_image: bool, is_image_array: bool) -> str:
        ...
        if is_image_array:
            ...
            return '<div class="grid grid-cols-5 gap-1 w-[620px]">%s</div>' % "".join(image_grid)
        
        rendered = CellRendererRegistry.render(value)
        return rendered if rendered is not None else str(value)
    
    def _get_cell_style(self, is_image: bool, is_image_array: bool) -> str:
        """获取单元格样式"""
        if is_image_array:
            return 'width: 620px; min-width: 620px; max-width: 620px; padding: 4px; vertical-align: top'
        elif is_image:
            return 'width: 120px; min-width: 120px; max-width: 120px; text-align: center; vertical-align: middle'
        else:
            return f'max-width: {self.max_column_width}px; word-break: break-word; white-space: normal; overflow-wrap: break-word'
    ...
    def to_html(self) -> str:
        ...
        # 获取基础单元格类名
        base_cell_classes = self._CELL_COMPACT_CLASSES if self.compact else self._CELL_BASE_CLASSES
        ...
        for col in self.columns:
            ...
            width_style = (
                'style="width: 650px; min-width: 650px; max-width: 650px;"' if is_image_array
                else 'style="width: 120px; min-width: 120px; max-width: 120px;"' if is_image
                else f'style="max-width: {self.max_column_width}px; min-width: {self.min_column_width}px;"'
            )
            
            sort_indicator = "↓" if self.sort_by == key and self.sort_desc else "↑" if self.sort_by == key else ""
            header_classes = [*self._HEADER_BASE_CLASSES, *base_cell_classes]
            
            header_cells.append('<th scope="col" class="%s" %s>%s %s</th>' % (
                ' '.join(header_classes), width_style, col.get("title", key), sort_indicator
            ))
        ...
        for i, row in enumerate(display_data):
            ...
            for col in self.columns:
                ...
                if not (is_image or is_image_array):
                    cell_classes.extend(["whitespace-normal", "break-words", "overflow-hidden"])
                
                style_attr = f'style="{self._get_cell_style(is_image, is_image_array)}"'
                
                cells.append('<td class="%s" %s>%s</td>' % (
                    ' '.join(cell_classes), style_attr, rendered_value
                ))
        ...
        </div>''' % (self.id, ' '.join(table_classes), ''.join(header_cells), ''.join(rows_html)) 