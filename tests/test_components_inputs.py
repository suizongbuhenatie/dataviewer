...
def test_text_input_with_params():
    """测试带参数的文本输入框"""
    text_input = TextInput(placeholder="请输入...", value="初始值", label="用户名")
...
def test_button_with_params():
    """测试带参数的按钮"""
    button = Button(text="提交", onclick="handleSubmit()", label="提交按钮")
...
def test_button_custom_attributes():
    """测试按钮自定义属性"""
    button = Button(text="自定义", custom_class="primary", data_testid="submit-btn")
...
def test_text_input_custom_attributes():
    """测试文本输入框自定义属性"""
    text_input = TextInput(type="password", required=True, maxlength=20)
